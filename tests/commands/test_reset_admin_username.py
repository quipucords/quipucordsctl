...
def test_reset_admin_username_run_with_prompt_flag(mocker, caplog):
    """Test reset_admin_username.run with --prompt flag ignores env var."""
    test_username = "promptuser"
    mocker.patch.object(
        reset_admin_username.podman_utils,
        "secret_exists",
        return_value=False,
    ...
    caplog.set_level(logging.DEBUG)
    args = argparse.Namespace(prompt=True)
    assert reset_admin_username.run(args)
    assert "The admin login username was successfully updated." == caplog.messages[-1]
    set_secret.assert_called_once_with(
        reset_admin_username.PODMAN_SECRET_NAME, test_username, False
    )
...
def test_reset_admin_username_prompt_returns_none(mocker, caplog):
    ...
    caplog.set_level(logging.ERROR)
    assert not reset_admin_username.run(argparse.Namespace())
    assert "The admin login username was not updated." == caplog.messages[0]
